...
var guard = require('@0xsequence/guard');
...
class GuardRemoteSigner extends RemoteSigner {
  constructor(address, hostname, isSequence = false, defaultChainId = network.ChainId.MAINNET) {
    super();
    this.isSequence = isSequence;
    this.defaultChainId = defaultChainId;
    this._guardd = void 0;
    this._address = void 0;
    this._guardd = new guard.GuarddService(hostname, fetchPonyfill__default["default"]().fetch);
  ...
  async signMessageWithData(message, auxData, chainId) {
    const request = {
      ...
      auxData: ethers.ethers.utils.hexlify(auxData ? auxData : []),
      chainId: chainId ? ethers.BigNumber.from(chainId).toNumber() : this.defaultChainId
    };
...
async function resolveArrayProperties(object) {
  if (Array.isArray(object)) {
    ...
    return Promise.all(object.map(o => properties.resolveProperties(o)));
  }

  return properties.resolveProperties(object);
}
...
class Wallet extends Signer {
  ...
  constructor(options, ...signers) {
    ...
    if (context) {
      ...
      this.context = _extends({}, network.sequenceContext);
    }
    ...
    if (!this.context.nonStrict && !config.isUsableConfig(config$1)) {
      throw new Error('wallet config is not usable (strict mode)');
    }

    this.config = config.sortConfig(config$1);
    this._signers = signers.map(s => ethers.Signer.isSigner(s) ? s : new ethers.ethers.Wallet(s)); // cache wallet config for future imageHash lookups
  ...
  connect(provider, relayer) {
    if (network.isJsonRpcProvider(provider)) {
      return new Wallet({
        config: this.config,
        context: this.context
  ...
  setProvider(provider) {
    ...
    if (providers.Provider.isProvider(provider)) {
      ...
      this.sender = new network.JsonRpcSender(provider);
    } else {
      const jsonProvider = new providers.JsonRpcProvider(provider);
      this.provider = jsonProvider;
      this.sender = new network.JsonRpcSender(jsonProvider);
    }
  ...
  get address() {
    return config.addressOf(this.config, this.context);
  } // imageHash is the unique hash of the WalletConfig


  get imageHash() {
    return config.imageHash(this.config);
  ...
  async getSigners() {
    ...
    return Promise.all(this._signers.map(s => s.getAddress().then(s => ethers.ethers.utils.getAddress(s))));
  ...
  async sendTransaction(transaction, chainId, allSigners, quote, callback) {
    ...
    if (callback) {
      const address = config.addressOf(signedTxs.config, signedTxs.context);
      const metaTxnHash = transactions.computeMetaTxnHash(address, signedTxs.chainId, ...signedTxs.transactions);
  ...
  async signTransactions(txs, chainId, allSigners) {
    ...
    let stx = await transactions.fromTransactionish(this.context, this.address, transaction); // If a transaction has 0 gasLimit and not revertOnError
    // compute all new gas limits

    if (stx.find(a => !a.revertOnError && ethers.ethers.BigNumber.from(a.gasLimit || 0).eq(ethers.ethers.constants.Zero))) {
    ...
    const providedNonce = transactions.readSequenceNonce(...stx);
    const nonce = providedNonce ? providedNonce : await this.getNonce();
    stx = transactions.appendNonce(stx, nonce); // Get transactions digest

    const digest = transactions.digestOfTransactionsNonce(nonce, ...stx); // Bundle with signature
  ...
  async signMessage(message, chainId, allSigners, isDigest = false) {
    const data = typeof message === 'string' && !message.startsWith('0x') ? ethers.ethers.utils.toUtf8Bytes(message) : message;
    return this.sign(data, isDigest, chainId, allSigners);
  }

  async signTypedData(domain, types, message, chainId, allSigners) {
    const signChainId = await this.getChainIdNumber(chainId);
    const domainChainId = domain.chainId ? ethers.BigNumber.from(domain.chainId).toNumber() : undefined;
    ...
    const hash = utils.encodeTypedDataDigest({
      domain,
      types,
      message
    });
  ...
  async subDigest(digest, chainId) {
    ...
    return ethers.ethers.utils.arrayify(utils.subDigestOf(this.address, solvedChainId, digest));
  ...
  async sign(msg, isDigest = true, chainId, allSigners) {
    ...
    const signChainId = await this.getChainIdNumber(chainId);
    const digest = isDigest ? msg : ethers.ethers.utils.keccak256(msg); // Generate sub-digest
        ...
        try {
          ...
          if (RemoteSigner.isRemoteSigner(signer)) {
            ...
            try {
              ...
              const isEOASignature = config.recoverEOASigner(subDigest, {
                weight: s.weight,
                signature: signature
              }) === s.address;
    ...
    const totalWeight = localSignature.signers.filter(config.isDecodedSigner).reduce((totalWeight, signer) => totalWeight + signer.weight, 0);

    if (totalWeight >= this.config.threshold) {
      return config.encodeSignature(localSignature);
    } // include local signatures for remote signers


    const remoteSigners = this._signers.filter(s => RemoteSigner.isRemoteSigner(s));

    const remoteSignature = await signWith(remoteSigners, this.packMsgAndSig(digest, config.encodeSignature(localSignature), signChainId)); // Aggregate both local and remote signatures

    return config.encodeSignature(config.joinSignatures(localSignature, remoteSignature));
  ...
  async updateConfig(config, nonce, publish = false, indexed, quote, callback) {
    ...
    }, config), await this.sendTransaction(transactions.appendNonce(txs, n), undefined, undefined, quote, callback)];
  ...
  async buildUpdateConfigTransaction(config$1, publish = false, indexed) {
    var _this2 = this;

    if (!this.context.nonStrict && !config.isUsableConfig(config$1)) throw new Error('wallet config is not usable (strict mode)');
    const isUpgradable = await async function () {
      try {
        const implementation = await _this2.provider.getStorageAt(_this2.address, ethers.ethers.utils.defaultAbiCoder.encode(['address'], [_this2.address]));
        return config.compareAddr(implementation, _this2.context.mainModuleUpgradable) === 0;
      } catch (_unused2) {
        return false;
      }
    }();
    const walletInterface = new abi.Interface(abi$1.walletContracts.mainModule.abi); // empirically, this seems to work for the tests:
    // const gasLimit = 100000 + 1800 * config.signers.length
    //
    // but we're going to play it safe with this instead:

    const gasLimit = 2 * (100000 + 1800 * config$1.signers.length);
    ...
    const mainModuleInterface = new abi.Interface(abi$1.walletContracts.mainModuleUpgradable.abi);
    const transaction = {
      delegateCall: false,
      revertOnError: true,
      gasLimit: ethers.ethers.constants.Zero,
      to: this.address,
      value: ethers.ethers.constants.Zero,
      data: mainModuleInterface.encodeFunctionData(mainModuleInterface.getFunction('updateImageHash'), [config.imageHash(config$1)])
    };
    const postTransaction = publish ? await this.buildPublishConfigTransaction(config$1, indexed) : [];
    const transactions$1 = [...preTransaction, transaction, ...postTransaction]; // If update config reguires a single transaction
    ...
    return [{
      ...
      to: this.address,
      value: ethers.ethers.constants.Zero,
      data: walletInterface.encodeFunctionData(walletInterface.getFunction('selfExecute'), [transactions.sequenceTxAbiEncode(transactions$1)])
    }];
  }

  buildPublishConfigTransaction(config$1, indexed = true, nonce) {
    const sequenceUtilsInterface = new abi.Interface(abi$1.walletContracts.sequenceUtils.abi);
    return [{
      delegateCall: false,
      revertOnError: true,
      gasLimit: ethers.ethers.constants.Zero,
      ...
      value: ethers.ethers.constants.Zero,
      nonce: nonce,
      data: sequenceUtilsInterface.encodeFunctionData(sequenceUtilsInterface.getFunction('publishConfig'), [this.address, config$1.threshold, config.sortConfig(config$1).signers.map(s => ({
        weight: s.weight,
        signer: s.address
      })), indexed])
    }];
  ...
  async buildPublishSignersTransaction(indexed = true, nonce, requireFreshSigners = []) {
    ...
    const sequenceUtilsInterface = new abi.Interface(abi$1.walletContracts.sequenceUtils.abi);
    const requireFreshSignersInterface = new abi.Interface(abi$1.walletContracts.requireFreshSigner.abi);
    const message = ethers.ethers.utils.randomBytes(32);
    ...
    const decodedSignature = config.decodeSignature(signature);
    const filteredSignature = config.encodeSignature({
      threshold: decodedSignature.threshold,
      signers: decodedSignature.signers.map((s, i) => {
        if (config.isDecodedFullSigner(s)) {
    ...
    return [...requireFreshSigners.map(signer => ({
      ...
      value: ethers.ethers.constants.Zero,
      nonce: nonce,
      data: sequenceUtilsInterface.encodeFunctionData(sequenceUtilsInterface.getFunction('publishInitialSigners'), [this.address, ethers.ethers.utils.keccak256(message), this.config.signers.length, filteredSignature, indexed])
    }];
  ...
  async getChainIdNumber(chainId) {
    ...
    const id = network.getChainId(chainId);
  ...
  packMsgAndSig(msg, sig, chainId) {
    return ethers.ethers.utils.defaultAbiCoder.encode(['address', 'uint256', 'bytes', 'bytes'], [this.address, chainId, msg, sig]);
  }
  ...
  static async singleOwner(owner, context) {
    const signer = ethers.Signer.isSigner(owner) ? owner : new ethers.ethers.Wallet(owner);
    const config = {
      threshold: 1,
      signers: [{
        weight: 1,
        address: ethers.ethers.utils.getAddress(await signer.getAddress())
      }]
    };
    return new Wallet({
      config,
...
class Account extends Signer {
  ...
  constructor(options, ...signers) {
    ...
    if (!options.context) this.options.context = _extends({}, network.sequenceContext); // Network config, defaults will be used if none are provided

    if (this.options.networks) {
      this.setNetworks(this.options.networks);
    } else {
      this.setNetworks([...network.mainnetNetworks]);
    }
  }
  ...
  getConfigFinder() {
    ...
    return new config.SequenceUtilsFinder(this.authWallet().wallet.provider);
  ...
  async getFeeOptions(transaction, chainId, allSigners = true) {
    ...
    const finalTransactions = await transactions.fromTransactionish(context, this.address, updatedTransaction);
  ...
  async prependConfigUpdate(dtransactionish, chainId, allSigners, skipThresholdCheck) {
    ...
    if ((await wallet.isDeployed()) && config.isConfigEqual(lastConfig, thisConfig)) {
      return transaction;
  ...
  async updateConfig(newConfig, index, quote, callback) {
    ...
    if (!newConfig) {
      ...
      newConfig = config.sortConfig(newConfig);
    } // The config is the default config, see if the wallet has been deployed


    if (config.isConfigEqual(authWallet.config, newConfig)) {
    ...
    if (config.isConfigEqual(lastConfig, newConfig)) {
  ...
  getWalletByNetwork(chainId) {
    const networkId = network.getChainId(chainId);

    const network$1 = this._wallets.find(w => w.network.chainId === networkId);
  ...
  setNetworks(mainnetNetworks, testnetNetworks = [], defaultChainId) {
    ...
    this.options.networks = network.ensureValidNetworks(network.sortNetworks(networks)); // Account/wallet instances using the initial configuration and network list
    //
    ...
    this._wallets = this.options.networks.map(network => {
      ...
      if (relayer.isRelayer(network.relayer)) {
        wallet.setRelayer(network.relayer);
      } else if (relayer.isRpcRelayerOptions(network.relayer)) {
        wallet.setRelayer(new relayer.RpcRelayer(_extends({
          provider: wallet.provider
        }, network.relayer)));
      } else {
...
function isValidEIP712Signature(address, digest, sig) {
  try {
    return config.compareAddr(ethers.ethers.utils.recoverAddress(digest, ethers.ethers.utils.splitSignature(sig)), address) === 0;
  } catch (_unused) {
    return false;
  }
}
function isValidEthSignSignature(address, digest, sig) {
  try {
    const subDigest = ethers.ethers.utils.keccak256(ethers.ethers.utils.solidityPack(['string', 'bytes32'], ['\x19Ethereum Signed Message:\n32', digest]));
    return config.compareAddr(ethers.ethers.utils.recoverAddress(subDigest, ethers.ethers.utils.splitSignature(sig)), address) === 0;
  } catch (_unused2) {
    return false;
  }
...
async function isValidContractWalletSignature(address, digest, sig, provider) {
  ...
  try {
    ...
    const wallet = new ethers.ethers.Contract(address, abi$1.walletContracts.erc1271.abi, provider);
    const response = await wallet.isValidSignature(digest, sig);
    return abi$1.walletContracts.erc1271.returns.isValidSignatureBytes32 === response;
  } catch (_unused3) {
    return false;
  }
}
async function isValidSequenceUndeployedWalletSignature(address, digest, sig, walletContext, provider, chainId) {
  ...
  try {
    ...
    const signature = config.decodeSignature(sig);
    const subDigest = ethers.ethers.utils.arrayify(ethers.ethers.utils.keccak256(utils.packMessageData(address, cid, digest)));
    const config$1 = await recoverConfigFromDigest(subDigest, signature, provider, walletContext, chainId, true);
    const weight = signature.signers.reduce((v, s) => config.isDecodedEOASigner(s) || config.isDecodedFullSigner(s) ? v + s.weight : v, 0);
    return config.compareAddr(config.addressOf(config$1, walletContext), address) === 0 && weight >= signature.threshold;
  } catch (_unused4) {
    return false;
  }
}
...
const fetchImageHash = async signer => {
  ...
  const walletContract = new ethers.Contract(address, abi$1.walletContracts.mainModuleUpgradable.abi, await signer.getProvider());
...
const recoverConfig = async (subDigest, signature, provider, context, chainId, walletSignersValidation) => {
  const digest = ethers.ethers.utils.arrayify(ethers.ethers.utils.keccak256(subDigest));
...
const recoverConfigFromDigest = async (digest, signature, provider, context, chainId, walletSignersValidation) => {
  const decoded = signature.threshold !== undefined ? signature : config.decodeSignature(signature);
  const signers = await Promise.all(decoded.signers.map(async s => {
    if (config.isDecodedEOASigner(s)) {
      return {
        weight: s.weight,
        address: config.recoverEOASigner(digest, s)
      };
    } else if (config.isDecodedAddress(s)) {
      return {
        weight: s.weight,
        address: ethers.ethers.utils.getAddress(s.address)
      };
    } else if (config.isDecodedFullSigner(s)) {
      if (walletSignersValidation) {
        if (!(await isValidSignature(s.address, ethers.ethers.utils.arrayify(digest), ethers.ethers.utils.hexlify(s.signature), provider, context, chainId))) throw Error('Invalid signature');
      }
...
exports.GuardRemoteSigner = GuardRemoteSigner;
exports.InvalidSigner = InvalidSigner;
exports.LocalRemoteSigner = LocalRemoteSigner;
exports.NotEnoughSigners = NotEnoughSigners;
exports.RemoteSigner = RemoteSigner;
exports.Signer = Signer;
exports.Wallet = Wallet;
exports.fetchImageHash = fetchImageHash;
exports.findLatestLog = findLatestLog;
exports.isSequenceSigner = isSequenceSigner;
...
exports.isValidEthSignSignature = isValidEthSignSignature;
exports.isValidSequenceUndeployedWalletSignature = isValidSequenceUndeployedWalletSignature;
exports.isValidSignature = isValidSignature;
exports.recoverConfig = recoverConfig;
exports.recoverConfigFromDigest = recoverConfigFromDigest;
