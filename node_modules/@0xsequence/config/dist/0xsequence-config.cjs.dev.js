...
var multicall = require('@0xsequence/multicall');
...
var multicall__namespace = /*#__PURE__*/_interopNamespace(multicall);
...
const createWalletConfig = async (threshold, signers) => {
  ...
  signers.forEach(async s => {
    config.signers.push({
      weight: s.weight,
      address: ethers.Signer.isSigner(s.signer) ? await s.signer.getAddress() : s.signer
    });
  });
...
const isUsableConfig = config => {
  const sum = config.signers.reduce((p, c) => ethers.ethers.BigNumber.from(c.weight).add(p), ethers.ethers.constants.Zero);
  return sum.gte(ethers.ethers.BigNumber.from(config.threshold));
};
const isValidConfigSigners = (config, signers) => {
  if (signers.length === 0) return true;
  const a = config.signers.map(s => ethers.ethers.utils.getAddress(s.address));
  const b = signers.map(s => ethers.ethers.utils.getAddress(s));
  let valid = true;
  b.forEach(s => {
...
const addressOf = (salt, context, ignoreAddress = false) => {
  if (typeof salt === 'string') {
    const codeHash = ethers.ethers.utils.keccak256(ethers.ethers.utils.solidityPack(['bytes', 'bytes32'], [WalletContractBytecode, ethers.ethers.utils.hexZeroPad(context.mainModule, 32)]));
    const hash = ethers.ethers.utils.keccak256(ethers.ethers.utils.solidityPack(['bytes1', 'address', 'bytes32', 'bytes32'], ['0xff', context.factory, salt, codeHash]));
    return ethers.ethers.utils.getAddress(ethers.ethers.utils.hexDataSlice(hash, 12));
  }
...
const imageHash = config => {
  ...
  const imageHash = config.signers.reduce((imageHash, signer) => ethers.ethers.utils.keccak256(ethers.ethers.utils.defaultAbiCoder.encode(['bytes32', 'uint8', 'address'], [imageHash, signer.weight, signer.address])), ethers.ethers.utils.solidityPack(['uint256'], [config.threshold]));
...
const sortConfig = config => {
  ...
  config.signers.forEach(s => s.address = ethers.ethers.utils.getAddress(s.address));
  if (config.address) config.address = ethers.ethers.utils.getAddress(config.address); // ensure no duplicate signers in the config
...
const compareAddr = (a, b) => {
  const bigA = ethers.ethers.BigNumber.from(a);
  const bigB = ethers.ethers.BigNumber.from(b);
...
function editConfig(config, args) {
  const normSigner = s => ({
    weight: ethers.ethers.BigNumber.from(s.weight).toNumber(),
  ...
  const normDelAddress = args.del ? args.del.map(a => ethers.ethers.utils.getAddress(a.address)) : [];
  ...
  return sortConfig({
    ...
    threshold: args.threshold ? ethers.ethers.BigNumber.from(args.threshold).toNumber() : config.threshold,
    signers: newSigners
  });
...
function genConfig(threshold, signers) {
  return sortConfig({
    threshold: ethers.ethers.BigNumber.from(threshold).toNumber(),
...
class SequenceUtilsFinder {
  constructor(authProvider) {
    ...
    this.findCurrentConfig = async function (args) {
      ...
      const address = ethers.ethers.utils.getAddress(args.address);
      utils.logger.info(`[findCurrentConfig] address:${address}, ignoreIndex:${ignoreIndex}, requireIndex:${requireIndex}`);
    ...
    this.findLastWalletOfInitialSigner = async function (args) {
      ...
      utils.logger.info(`[findLastWalletOfInitialSigner] signer:${signer}`);
      if (requireIndex && ignoreIndex) throw Error(`findCurrentConfig: can't ignore index and require index`);
      const authContract = new ethers.Contract(context.sequenceUtils, abi.walletContracts.sequenceUtils.abi, _this.authProvider);
      ...
      if (lastLog === undefined) {
        utils.logger.warn('publishConfig: wallet config last log not found');
        return {
          wallet: undefined
        };
      }
    ...
    this.findConfigForImageHash = async function (context, image, knownConfigs = [], skipCache = false) {
      ...
      utils.logger.info(`[findConfigForImageHash] image:${image}`); // Load index for last imageHash update
      ...
      const signers = ethers.ethers.utils.defaultAbiCoder.decode([`tuple(
        uint256 weight,
        address signer
      )[]`], event._signers)[0];
      const config = {
        threshold: ethers.ethers.BigNumber.from(event._threshold).toNumber(),
        signers: signers.map(s => ({
          address: s.signer,
          weight: ethers.ethers.BigNumber.from(s.weight).toNumber()
        }))
    ...
    this.findCurrentImageHash = async function (context, provider, address, knownConfigs = [], skipCache) {
      utils.logger.info(`[findCurrentImageHash] address:${address}`);
      const walletContract = new ethers.Contract(address, abi.walletContracts.mainModuleUpgradable.abi, provider);
      ...
      const normalizedAddress = ethers.ethers.utils.getAddress(address);
      ...
      if (knownImageHash !== ethers.ethers.constants.HashZero) {
    ...
    this.findLatestLog = async function (provider, filter) {
      ...
      if (fromBlock === 0) {
        utils.logger.warn(`findLatestLog: expensive getLogs query fromBlock 0 toBlock ${toBlock}`);
      }
    ...
    this.findFirstLog = async function (provider, filter) {
      ...
      if (fromBlock === 0) {
        utils.logger.warn(`findFirstLog: expensive getLogs query fromBlock 0 toBlock ${toBlock}`);
      }
...
const decodeSignature = signature => {
  ...
  const threshold = ethers.ethers.BigNumber.from(`0x${auxsig.slice(0, 4)}`).toNumber();
  const signers = [];

  for (let rindex = 4; rindex < auxsig.length;) {
    const signatureType = ethers.ethers.BigNumber.from(auxsig.slice(rindex, rindex + 2)).toNumber();
    rindex += 2;
    const weight = ethers.ethers.BigNumber.from(`0x${auxsig.slice(rindex, rindex + 2)}`).toNumber();
    rindex += 2;

    switch (signatureType) {
      case exports.SignatureType.Address:
        const addr = ethers.ethers.utils.getAddress(auxsig.slice(rindex, rindex + 40));
        rindex += 40;
        signers.push({
          weight: weight,
          address: addr
      ...
      case exports.SignatureType.EOA:
        const sig = ethers.ethers.utils.arrayify(`0x${auxsig.slice(rindex, rindex + 132)}`);
        rindex += 132;
        const split = ethers.ethers.utils.splitSignature(sig.slice(0, 65));
        const r = split.r;
        const s = split.s;
        const v = split.v;
        const t = ethers.ethers.BigNumber.from(sig[sig.length - 1]).toNumber();
        signers.push({
          weight: weight,
          signature: sig,
          r: r,
      ...
      case exports.SignatureType.Full:
        const address = ethers.ethers.utils.getAddress(auxsig.slice(rindex, rindex + 40));
        rindex += 40;
        const size = ethers.ethers.BigNumber.from(`0x${auxsig.slice(rindex, rindex + 4)}`).mul(2).toNumber();
        rindex += 4;

        const _signature = ethers.ethers.utils.arrayify(`0x${auxsig.slice(rindex, rindex + size)}`);
...
const splitDecodedEOASigner = sig => {
  const signature = ethers.ethers.utils.arrayify(sig.signature);
  const split = ethers.ethers.utils.splitSignature(signature.slice(0, 65));
  const t = ethers.ethers.BigNumber.from(signature[signature.length - 1]).toNumber();
...
const recoverEOASigner = (digest, sig) => {
  ...
  switch (signature.t) {
    case SIG_TYPE_EIP712:
      return ethers.ethers.utils.recoverAddress(digest, {
        r: signature.r,
        s: signature.s,
        v: signature.v
      });
    ...
    case SIG_TYPE_ETH_SIGN:
      ...
      return ethers.ethers.utils.recoverAddress(subDigest, {
        r: signature.r,
        s: signature.s,
        v: signature.v
      });
...
const encodeSignature = sig => {
  ...
  const accountBytes = sig.signers.map(s => {
    if (isDecodedAddress(s)) {
      return ethers.ethers.utils.solidityPack(['uint8', 'uint8', 'address'], [exports.SignatureType.Address, s.weight, s.address]);
    }

    if (isDecodedEOASplitSigner(s)) {
      return ethers.ethers.utils.solidityPack(['uint8', 'uint8', 'bytes32', 'bytes32', 'uint8', 'uint8'], [exports.SignatureType.EOA, s.weight, s.r, s.s, s.v, s.t]);
    }

    if (isDecodedFullSigner(s)) {
      const signatureSize = ethers.ethers.utils.arrayify(s.signature).length;
      return ethers.ethers.utils.solidityPack(['uint8', 'uint8', 'address', 'uint16', 'bytes'], [exports.SignatureType.Full, s.weight, s.address, signatureSize, s.signature]);
    }

    if (isDecodedEOASigner(s)) {
      return ethers.ethers.utils.solidityPack(['uint8', 'uint8', 'bytes'], [exports.SignatureType.EOA, s.weight, s.signature]);
    }

    throw Error('Unkwnown signature part type');
  });
  return ethers.ethers.utils.solidityPack(['uint16', ...Array(accountBytes.length).fill('bytes')], [sig.threshold, ...accountBytes]);
};
...
async function buildStubSignature(provider, config) {
  const multicallProvider = new multicall__namespace.providers.MulticallProvider(provider); // Pre-load if signers are EOAs or not

  const signers = await Promise.all(config.signers.map(async (s, i) => {
    return _extends({}, s, {
      index: i,
      isEOA: ethers.ethers.utils.arrayify(await multicallProvider.getCode(s.address)).length === 0
    });
  ...
  const stubSig = ethers.ethers.utils.arrayify("0x1fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff7fffffffffffffffffffffffffffffff5d576e7357a4501ddfe92f46681b20a01b02"); // Re-sort signers by original index
    ...
    signers: finalSigners.map(s => {
      ...
      return {
        ...
        signature: encodeSignature({
          threshold: 1,
          signers: [{
            address: ethers.ethers.Wallet.createRandom().address,
            weight: 1
          }, {
            weight: 1,
            signature: stubSig
          }]
        }) + ethers.ethers.utils.hexlify(SIG_TYPE_WALLET_BYTES32).substring(2)
      };
    })
  };
}
...
exports.SequenceUtilsFinder = SequenceUtilsFinder;
exports.WalletContractBytecode = WalletContractBytecode;
exports.addressOf = addressOf;
exports.buildStubSignature = buildStubSignature;
exports.compareAddr = compareAddr;
...
exports.isConfigEqual = isConfigEqual;
exports.isDecodedAddress = isDecodedAddress;
exports.isDecodedEOASigner = isDecodedEOASigner;
exports.isDecodedEOASplitSigner = isDecodedEOASplitSigner;
exports.isDecodedFullSigner = isDecodedFullSigner;
exports.isDecodedSigner = isDecodedSigner;
exports.isUsableConfig = isUsableConfig;
exports.isValidConfigSigners = isValidConfigSigners;
exports.joinSignatures = joinSignatures;
exports.joinTwoSignatures = joinTwoSignatures;
exports.mutateSignature = mutateSignature;
exports.recoverEOASigner = recoverEOASigner;
exports.signerOf = signerOf;
exports.sortConfig = sortConfig;
exports.splitDecodedEOASigner = splitDecodedEOASigner;
