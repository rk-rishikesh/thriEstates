...
class Session {
  ...
  async auth(maxTries = 5) {
    ...
    return new api.SequenceAPIClient(url, jwtAuth);
  }
  ...
  async getAPIClient(tryAuth = true) {
    if (!this.apiClient) {
      ...
      this.apiClient = new api.SequenceAPIClient(url, jwtAuth);
    }
  ...
  getMetadataClient() {
    if (!this.metadataClient) {
      this.metadataClient = new metadata.SequenceMetadataClient(this.sequenceMetadataUrl);
    }
  ...
  async getIndexerClient(chainId) {
    const network$1 = network.findNetworkConfig(this.networks, chainId);
    ...
    if (!this.indexerClients.has(network$1.chainId)) {
      if (network$1.indexer) {
        ...
        this.indexerClients.set(network$1.chainId, new indexer.SequenceIndexerClient(network$1.indexerUrl, jwtAuth));
      } else {
  ...
  getProofString(key) {
    ...
    const configFinder = new config.SequenceUtilsFinder(this.authProvider);
    ...
    const proofString = {
      ...
      proofString: configFinder.findCurrentConfig({
        ...
        return authWallet.wallet.useConfig(val.config).sign(proof.messageDigest()).then(signature => {
          ...
          const totalWeight = decodedSignature.signers.filter(config.isDecodedSigner).reduce((totalWeight, signer) => totalWeight + signer.weight, 0);
  ...
  static async open(args) {
    ...
    const configFinder = args.configFinder ? args.configFinder : new config.SequenceUtilsFinder(authProvider);
    ...
    if (existingWallet) {
      ...
      const config$1 = (await configFinder.findCurrentConfig({
        address: existingWallet,
        provider: authProvider,
        context: context,
        knownConfigs
      })).config;
      if (!config$1) throw Error('Wallet config not found'); // Load prev account

      const account = new wallet.Account({
        initialConfig: config$1,
        networks: networks,
        context: context
      }, ...fullSigners);
      const session = new Session(sequenceApiUrl, sequenceMetadataUrl, networks, config$1, context, account, metadata, authProvider); // Update wallet config on-chain on the authChain

      const [newConfig] = await account.updateConfig(config.editConfig(config$1, {
        threshold,
        set: await solvedSigners
      }), noIndex ? false : true); // Session is ready, lets update

      session.setConfig(newConfig);
      session.setAccount(new wallet.Account({
      ...
      // fresh account
      const config$1 = config.genConfig(threshold, await solvedSigners);
      const account = new wallet.Account({
        initialConfig: config$1,
        networks: networks,
        context: context
      ...
      const session = new Session(sequenceApiUrl, sequenceMetadataUrl, networks, config$1, context, account, metadata, authProvider);
  ...
  static load(args) {
    ...
    return new Session(sequenceApiUrl, sequenceMetadataUrl, networks, dump.config, dump.context, new wallet.Account({
...
function getAuthProvider(networks) {
  ...
  const authChain = network.getAuthNetwork(networks);
  if (!authChain) throw Error('Auth chain not found');
  return (_authChain$provider = authChain.provider) != null ? _authChain$provider : new ethers.ethers.providers.JsonRpcProvider(authChain.rpcUrl);
}

function getJWTExpiration(jwt) {
  return utils.jwtDecodeClaims(jwt).exp;
}
...
const ValidateSequenceDeployedWalletProof = async (provider, chainId, proof) => {
  ...
  const contract = new ethers.ethers.Contract(proof.address, abi, provider); // hash the message digest as required by isValidSignature
...
const ValidateSequenceUndeployedWalletProof = context => {
  return async (provider, chainId, proof) => {
    ...
    const walletCode = ethers.ethers.utils.arrayify(await provider.getCode(proof.address));
    ...
    const digest = ethers.ethers.utils.arrayify(ethers.ethers.utils.keccak256(message));
    const isValid = await wallet.isValidSequenceUndeployedWalletSignature(proof.address, digest, proof.signature, context ? context : network.sequenceContext, provider, chainId);
    return {
      isValid: !!isValid
    };
  };
...
exports.DEFAULT_SESSION_EXPIRATION = DEFAULT_SESSION_EXPIRATION;
exports.ErrAccountIsRequired = ErrAccountIsRequired;
exports.LONG_SESSION_EXPIRATION = LONG_SESSION_EXPIRATION;
exports.Session = Session;
exports.ValidateSequenceDeployedWalletProof = ValidateSequenceDeployedWalletProof;
exports.ValidateSequenceUndeployedWalletProof = ValidateSequenceUndeployedWalletProof;
