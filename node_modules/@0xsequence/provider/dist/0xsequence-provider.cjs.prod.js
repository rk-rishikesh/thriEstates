...
})(exports.OpenState || (exports.OpenState = {}));
...
class BaseProviderTransport {
  constructor() {
    ...
    this.sendAsync = async function (request, callback, chainId) {
      ...
      if (_this.state === exports.OpenState.CLOSED) {
      ...
      try {
        ...
        const response = await _this.sendMessageRequest({
          idx: nextMessageIdx(),
          type: exports.EventType.MESSAGE,
          data: request,
          chainId: chainId
        });
    ...
    this.waitUntilOpened = async function (openTimeout = PROVIDER_OPEN_TIMEOUT) {
      ...
      return Promise.race([new Promise((_, reject) => {
        const timeout = setTimeout(() => {
          ...
          if (!opened) {
            _this.state = exports.OpenState.CLOSED;
    ...
    this.state = exports.OpenState.CLOSED;
    this._registered = false;
    this._init = exports.InitState.NIL;
  }
  ...
  isOpened() {
    return this.registered && this.state === exports.OpenState.OPENED;
  }
  ...
  handleMessage(message) {
    ...
    if (message.type === exports.EventType.INIT) {
      ...
      this._init = exports.InitState.OK;
      this.sendMessage({
        idx: -1,
        type: exports.EventType.INIT,
        data: {
    ...
    if (this._init !== exports.InitState.OK) {
    ...
    if (message.type === exports.EventType.OPEN && !this.isOpened()) {
      ...
      this.state = exports.OpenState.OPENED;
    ...
    if (message.type === exports.EventType.MESSAGE) {
    ...
    if (message.type === exports.EventType.ACCOUNTS_CHANGED) {
      ...
      if (message.data && message.data.length > 0) {
        this.accountsChangedPayload = {
          accounts: [ethers.ethers.utils.getAddress(message.data[0])],
    ...
    if (message.type === exports.EventType.CHAIN_CHANGED) {
      this.events.emit('chainChanged', message.data);
      return;
    } // NOTIFY NETWORKS -- when a user connects or logs in


    if (message.type === exports.EventType.NETWORKS) {
    ...
    if (message.type === exports.EventType.WALLET_CONTEXT) {
    ...
    if (message.type === exports.EventType.CLOSE) {
      if (this.state !== exports.OpenState.CLOSED) {
        this.close(message.data);
      }
    } // NOTIFY CONNECT -- when wallet instructs we've connected


    if (message.type === exports.EventType.CONNECT) {
    ...
    if (message.type === exports.EventType.DISCONNECT) {
      if (this.isConnected()) {
  ...
  close(error) {
    if (this.state === exports.OpenState.CLOSED) return;
    this.state = exports.OpenState.CLOSED;
    this.confirmationOnly = false;
    this._sessionId = undefined;
    utils.logger.info('closing wallet and flushing!'); // flush pending requests and return error to all callbacks
...
class WalletRequestHandler {
      ...
      try {
        ...
        switch (request.method) {
          ...
          case 'eth_sign':
            {
              ...
              if (_this.prompter === null) {
                ...
                sig = await signer.signMessage(ethers.ethers.utils.arrayify(message), chainId);
              } else {
          ...
          case 'eth_signTransaction':
            {
              ...
              const sender = ethers.ethers.utils.getAddress(transaction.from);
          ...
          case 'eth_sendRawTransaction':
            {
              ...
              if (transactions.isSignedTransactions(request.params[0])) {
                const txChainId = ethers.BigNumber.from(request.params[0].chainId).toNumber();
          ...
          case 'eth_getTransactionCount':
            {
              const address = ethers.ethers.utils.getAddress(request.params[0]);
              const tag = request.params[1];
              const walletAddress = ethers.ethers.utils.getAddress(await signer.getAddress());

              if (address === walletAddress) {
                const count = await signer.getTransactionCount(tag);
                response.result = ethers.ethers.BigNumber.from(count).toHexString();
              } else {
          ...
          case 'wallet_switchEthereumChain':
            {
              ...
              const _chainId = ethers.ethers.BigNumber.from(switchParams.chainId);
        ...
        utils.logger.error(err); // See https://github.com/ethereum/EIPs/blob/master/EIPS/eip-1193.md#rpc-errors
  ...
  async connect(options) {
    ...
    const connectDetails = {
      connected: true,
      chainId: ethers.ethers.utils.hexlify(await this.getChainId())
    };
    ...
    if (options && options.authorize) {
      ...
      try {
        connectDetails.proof = await auth.signAuthorization(this.signer, authOptions);
      } catch (err) {
  ...
  async notifyNetworks(networks) {
    ...
    if (n.length > 0) {
      ...
      if (defaultNetwork) {
        this.events.emit('chainChanged', ethers.ethers.utils.hexlify(defaultNetwork.chainId));
      }
    } else {
...
class Web3Provider extends providers.Web3Provider {
  ...
  constructor(provider, defaultChainId) {
    const sender = new network.JsonRpcSender(provider, network.maybeChainId(defaultChainId));
    provider = sender;
    super(provider, 'any');
    this._sender = void 0;
    ...
    this._defaultChainId = network.maybeChainId(defaultChainId);
  }
  ...
  async getChainId() {
    ...
    const chainId = ethers.ethers.BigNumber.from(result).toNumber();
...
class Web3Signer extends wallet.Signer {
  constructor(provider, defaultChainId) {
    super();
  ...
  async getAddress() {
    ...
    this._index = 0;
    return ethers.ethers.utils.getAddress(this._address);
  }
  ...
  async getSender(chainId) {
    ...
    if (!this._providers[chainId]) {
      this._providers[chainId] = new Web3Provider(new network.JsonRpcSender(this.provider, chainId), chainId);
    }
  ...
  async getWalletConfig(chainId) {
    return await this.provider.send('sequence_getWalletConfig', [network.maybeChainId(chainId)], network.maybeChainId(chainId) || this.defaultChainId);
  }

  async getWalletState(chainId) {
    return await this.provider.send('sequence_getWalletState', [network.maybeChainId(chainId)], network.maybeChainId(chainId) || this.defaultChainId);
  }
  ...
  async signMessage(message, chainId, allSigners) {
    const provider = await this.getSender(network.maybeChainId(chainId) || this.defaultChainId);
    const data = typeof message === 'string' ? ethers.ethers.utils.toUtf8Bytes(message) : message;
    ...
    return await provider.send('personal_sign', [ethers.ethers.utils.hexlify(data), address]);
  ...
  async signTypedData(domain, types, message, chainId, allSigners) {
    ...
    // })
    return await this.provider.send('eth_signTypedData_v4', [await this.getAddress(), ethers.ethers.utils._TypedDataEncoder.getPayload(domain, types, message)], network.maybeChainId(chainId) || this.defaultChainId);
  ...
  async sendTransaction(transaction, chainId, allSigners) {
    ...
    const tx = this.sendUncheckedTransaction(transaction, chainId).then(hash => {
      return ethers.ethers.utils.poll(() => {
  ...
  async sendTransactionBatch(transactions, chainId, allSigners) {
    const batch = await wallet.resolveArrayProperties(transactions);
  ...
  signTransactions(transaction, chainId, allSigners) {
    transaction = utils.shallowCopy(transaction); // TODO: transaction argument..? make sure to resolve any properties and serialize property before sending over
    // the wire.. see sendUncheckedTransaction and resolveProperties

    return this.provider.send('eth_signTransaction', [transaction], network.maybeChainId(chainId) || this.defaultChainId);
  }
  ...
  async isDeployed(chainId) {
    const provider = await this.getSender(network.maybeChainId(chainId));
  ...
  async _legacySignMessage(message, chainId, allSigners) {
    ...
    return await provider.send('eth_sign', [address, ethers.ethers.utils.hexlify(data)]);
  }
  ...
  async sendUncheckedTransaction(transaction, chainId) {
    ...
    return utils.resolveProperties({
      tx: utils.resolveProperties(transaction),
      ...
      if (tx.from != null) {
        if (ethers.ethers.utils.getAddress(tx.from) !== sender) {
...
const hexlifyTransaction = (transaction, allowExtra) => {
  ...
  const allowed = utils.shallowCopy(allowedTransactionKeys);
  ...
  ethers.ethers.utils.checkProperties(transaction, allowed);
  ...
  ['gasLimit', 'gasPrice', 'nonce', 'value'].forEach(key => {
    ...
    const value = ethers.ethers.utils.hexValue(transaction[key]);
  ...
  ['from', 'to', 'data'].forEach(key => {
    ...
    result[key] = ethers.ethers.utils.hexlify(transaction[key]);
  });
...
const eip191prefix = ethers.ethers.utils.toUtf8Bytes('\x19Ethereum Signed Message:\n');
const messageToBytes = message => {
  if (ethers.ethers.utils.isBytes(message) || ethers.ethers.utils.isHexString(message)) {
    return ethers.ethers.utils.arrayify(message);
  }

  return ethers.ethers.utils.toUtf8Bytes(message);
};
const prefixEIP191Message = message => {
  const messageBytes = messageToBytes(message);
  return ethers.ethers.utils.concat([eip191prefix, ethers.ethers.utils.toUtf8Bytes(String(messageBytes.length)), messageBytes]);
};
const isValidSignature = async (address, digest, sig, provider, chainId, walletContext) => {
  ...
  return wallet.isValidSignature(address, digest, sig, provider, walletContext, chainId);
};
...
const isValidTypedDataSignature = (address, typedData, signature, provider, chainId, walletContext) => {
  return isValidSignature(address, utils.encodeTypedDataDigest(typedData), signature, provider, chainId, walletContext);
};
const recoverWalletConfig = async (address, digest, signature, chainId, walletContext) => {
  const subDigest = utils.packMessageData(address, chainId, digest);
  const config$1 = await wallet.recoverConfig(subDigest, signature);

  if (walletContext) {
    const recoveredWalletAddress = config.addressOf(config$1, walletContext);

    if (config$1.address && config$1.address !== recoveredWalletAddress) {
      throw new Error('recovered address does not match the WalletConfig address, check the WalletContext');
    } else {
      config$1.address = recoveredWalletAddress;
    }
  }
...
const isWalletUpToDate = async (signer, chainId) => {
  ...
  const isUpToDate = config.isConfigEqual(authChainConfig, requiredChainConfig);
...
class BaseWalletTransport {
  ...
  constructor(walletRequestHandler) {
    ...
    this.handleMessage = async function (message) {
      ...
      if (_this._init !== exports.InitState.OK) {
        if (request.type === exports.EventType.INIT) {
      ...
      switch (request.type) {
        case exports.EventType.OPEN:
          {
            if (_this._init !== exports.InitState.OK) return;
            const session = {
        ...
        case exports.EventType.CLOSE:
          {
            if (_this._init !== exports.InitState.OK) return; // noop. just here to capture the message so event emitters may be notified

            return;
          }

        case exports.EventType.MESSAGE:
          {
    ...
    this.open = async function ({
      ...
      if (sessionId) {
        _this._sessionId = utils.sanitizeNumberString(sessionId); // persist transport session in localstorage for restoring after redirect/reload
    ...
    this.walletRequestHandler.on('networks', networks => {
      ...
      if (!networks || networks.length === 0) {
        ...
        this.notifyChainChanged(ethers.ethers.utils.hexlify(networks.find(network => network.isDefaultChain).chainId));
      }
    });
  ...
  notifyConnect(connectDetails) {
    this.sendMessage({
      idx: -1,
      type: exports.EventType.CONNECT,
      data: connectDetails
    });
  }
  ...
  notifyDisconnect(error) {
    this.sendMessage({
      idx: -1,
      type: exports.EventType.DISCONNECT,
      data: error ? {
        error
      } : null
    });
  ...
  notifyAccountsChanged(accounts, origin) {
    this.sendMessage({
      idx: -1,
      type: exports.EventType.ACCOUNTS_CHANGED,
      data: accounts,
      origin: origin
    });
  }
  ...
  notifyChainChanged(chainIdHex) {
    this.sendMessage({
      idx: -1,
      type: exports.EventType.CHAIN_CHANGED,
      data: chainIdHex
    });
  }
  ...
  notifyNetworks(networks) {
    this.sendMessage({
      idx: -1,
      type: exports.EventType.NETWORKS,
      data: networks
    });
  }
  ...
  notifyWalletContext(walletContext) {
    this.sendMessage({
      idx: -1,
      type: exports.EventType.WALLET_CONTEXT,
      data: walletContext
    });
  }
  ...
  init() {
    return new Promise((resolve, reject) => {
      ...
      if (this._init !== exports.InitState.NIL || this._initCallback) {
      ...
      this._init = exports.InitState.SENT_NONCE; // NOTE: the promise will resolve in the _initCallback method
...
class ProxyMessageProvider extends BaseProviderTransport {
  constructor(port) {
    ...
    this.openWallet = (path, intent, networkId) => {
      if (this.state === exports.OpenState.CLOSED) {
        this.state = exports.OpenState.OPENING;
...
class WindowMessageHandler extends BaseWalletTransport {
  constructor(walletRequestHandler) {
    ...
    this.onWindowEvent = async function (event) {
      ...
      utils.logger.debug('RECEIVED MESSAGE', request); // Record event origin for valid init ack

      if (_this._init !== exports.InitState.OK && _this.isValidInitAck(request)) {
        _this.appOrigin = event.origin;
      }

      if (_this._init === exports.InitState.OK && (!_this.appOrigin || _this.appOrigin.length < 8)) {
        // impossible state
  ...
  postMessage(message, init = false) {
    if (init !== true && this._init !== exports.InitState.OK) {
  ...
  constructor(stream) {
    ...
    this.handleMessage = message => {
      ...
      switch (message.type) {
        case exports.EventType.MESSAGE:
          if (responseCallback) {
            this.emit(exports.EventType.MESSAGE, message);
            responseCallback(message.data.error, message);
          } else {
            // NOTE: this would occur if 'idx' isn't set, which should never happen
            // or when we register two handler, or duplicate messages with the same idx are sent,
        ...
        case exports.EventType.DISCONNECT:
        case exports.EventType.ACCOUNTS_CHANGED:
        case exports.EventType.CHAIN_CHANGED:
...
class WalletUtils {
  constructor(walletProvider) {
    ...
    this.recoverWalletConfigFromMessage = async function (address, message, signature, chainId, walletContext) {
      walletContext = walletContext || (await _this.wallet.getWalletContext());
      return recoverWalletConfig(address, utils.encodeMessageDigest(prefixEIP191Message(message)), signature, chainId, walletContext);
    };

    this.recoverWalletConfigFromTypedData = async function (address, typedData, signature, chainId, walletContext) {
      walletContext = walletContext || (await _this.wallet.getWalletContext());
      return recoverWalletConfig(address, utils.encodeTypedDataDigest(typedData), signature, chainId, walletContext);
    };
  ...
  isValidTypedDataSignature(address, typedData, signature, chainId, walletContext) {
    return this.isValidSignature(address, utils.encodeTypedDataDigest(typedData), signature, chainId, walletContext);
...
class Wallet {
  constructor(_network, config) {
    ...
    this.init = () => {
      ...
      this.transport.allowProvider = network.allowProviderMiddleware(request => {
      ...
      this.transport.networkProvider = network.networkProviderMiddleware(request => {
      ...
      this.transport.cachedProvider = new network.CachedProvider();
      ...
      this.transport.router = new network.JsonRpcRouter([network.loggingProviderMiddleware, this.transport.networkProvider, this.transport.allowProvider, network.exceptionProviderMiddleware, this.transport.cachedProvider], this.transport.messageProvider);
    ...
    this.loadSession = async function () {
      ...
      try {
        ...
        utils.logger.warn('loadSession failed, unable to parse session payload from storage.');
        return undefined;
      }
    };
    ...
    this.connect = async function (options) {
      ...
      if (_this.isConnected() && (await _this.isSiteConnected(options == null ? void 0 : options.origin)) && !!_this.session && !(options != null && options.authorize) && !(options != null && options.askForEmail)) {
        return {
          ...
          chainId: ethers.ethers.utils.hexlify(await _this.getChainId())
        };
      }
    ...
    this.getNetworks = async function (chainId) {
      ...
      if (chainId) {
        ...
        const network$1 = network.findNetworkConfig(_this.networks, chainId);
    ...
    this.saveSession = async function (session) {
      utils.logger.debug('wallet provider: saving session');
  ...
  getProvider(chainId) {
    ...
    let network$1 = this.networks.find(network => network.isDefaultChain);

    if (chainId) {
      network$1 = network.findNetworkConfig(this.networks, chainId);
    ...
    if (this.providers[network$1.chainId]) {
      return this.providers[network$1.chainId];
    } // builder web3 provider stack


    let provider; // network.provider may be set by the ProviderConfig override

    const rpcProvider = network$1.provider ? network$1.provider : new providers.JsonRpcProvider(network$1.rpcUrl, network$1.chainId);

    if (network$1.isDefaultChain) {
      // communicating with defaultChain will prioritize the wallet message transport
      const router = new network.JsonRpcRouter([network.loggingProviderMiddleware, network.exceptionProviderMiddleware, new network.EagerProvider({
        accountAddress: this.session.accountAddress,
        walletContext: this.session.walletContext
      }), new network.SigningProvider(this.transport.provider), this.transport.cachedProvider], new network.JsonRpcSender(rpcProvider));
      provider = new Web3Provider(router, network$1.chainId);
    } else {
      ...
      }), new network.SigningProvider(this.transport.provider), new network.CachedProvider(network$1.chainId)], new network.JsonRpcSender(rpcProvider));
      provider = new Web3Provider(router, network$1.chainId);
    }

    this.providers[network$1.chainId] = provider;
    return provider;
  }
  ...
  useAccountAddress(accountAddress) {
    if (!this.session) this.session = {};
    this.session.accountAddress = ethers.ethers.utils.getAddress(accountAddress);
  }
  ...
  useNetworks(networks) {
    ...
    if (this.config.networks) {
      ...
      this.config.networks.forEach(n => {
        const network$1 = network.findNetworkConfig(this.networks, n.chainId || n.name);
        if (!network$1) return;
        network.updateNetworkConfig(n, network$1);
      });
      network.ensureValidNetworks(this.networks, true);
  ...
  clearSession() {
    ...
    utils.logger.debug('wallet provider: clearing session');
...
exports.LocalWeb3Provider = LocalWeb3Provider;
...
exports.isSequenceProvider = isSequenceProvider;
...
exports.recoverWalletConfig = recoverWalletConfig;
